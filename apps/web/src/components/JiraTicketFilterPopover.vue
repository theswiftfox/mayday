...
const filter = computed(() => prefs.filters.jiraTicket)
...
function toggleCategory(cat: string) {
  const cats = [...filter.value.statusCategories]
  const idx = cats.indexOf(cat)
  if (idx >= 0) cats.splice(idx, 1)
  else cats.push(cat)
  prefs.updateJiraTicketFilter({ statusCategories: cats })
}
...
function clearAll() {
  prefs.updateJiraTicketFilter({ statusCategories: [], priorities: [], issueTypes: [] })
}
    ...
    <div class="space-y-3">
      ...
      <div>
        ...
        <label v-for="cat in allCategories" :key="cat" class="flex items-center gap-2 py-0.5 cursor-pointer">
          <input type="checkbox" :checked="filter.statusCategories.includes(cat)" @change="toggleCategory(cat)" class="rounded" />
